                            ...
                            void          *d)
{
    ...
    for (; aIndexStart < aIndexHigh; aIndexStart++)
    {
        ...
        while (sLeft < sRight)
        {
            ...
            if ((*aCmpCb)(aState->mPivot, sArray + sMiddle * sWidth, d) == -1)
            {
                sRight = sMiddle;
            }
            else
                             ...
                             void          *d)
{
    ...
    if ((*aCmpCb)(aKey, aArray + (aBase + aHint) * aWidth, d) == 1)
    {
        /*
         * key > a[b+h]
        ...
        {
            if ((*aCmpCb)(aKey, aArray + (aBase + aHint + sOffset) * aWidth, d) == 1)
            {
                sLastOffset = sOffset;
                sOffset     = (sOffset << 1) + 1;
    ...
    while (sLastOffset < sOffset)
    {
        ...
        if ((*aCmpCb)(aKey, aArray + (aBase + sMiddle) * aWidth, d) == 1)
        {
            /* a[b+m] < key */
                        ...
                        void          *d)
{
    ...
    while (1)
    {
        ...
        do  /* Normal merge : left to right */
        {
            ...
            if ((*aCmpCb)(sArray + sCursor2 * sWidth, sTmp + sCursor1 * sWidth, d) == -1)
            {
                COPY(sArray + sDestIndex * sWidth, sArray + sCursor2 * sWidth, sWidth);
                sDestIndex++;
                sCursor2++;
                         ...
                         void          *d)
{
    ...
    while (1)
    {
        ...
        do  /* Normal merge : right to left */
        {
            ...
            if ((*aCmpCb)(sTmp + sCursor2 * sWidth, sArray + sCursor1 * sWidth, d) == -1)
            {
                COPY(sArray + sDestIndex * sWidth, sArray + sCursor1 * sWidth, sWidth);
                sDestIndex--;
                sCursor1--;
 ...
 *       i == PendingRuncnt - 3.
 */
static void timMergeAt(timMergeState *aState, uint32_t aWhere, cmpFunc *aCmpCb, void *d)
{
    int32_t sBaseA;
    int32_t sLenA;
    int32_t sBaseB;
    ...
    if (sLenA <= sLenB)
    {
        timMergeLow(aState, sBaseA, sLenA, sBaseB, sLenB, aCmpCb, d);
    }
    else
    {
        timMergeHigh(aState, sBaseA, sLenA, sBaseB, sLenB, aCmpCb, d);
    }
}
 ...
 * entry to the method.
 */
static void timMergeCollapse(timMergeState *aState, cmpFunc *aCmpCb, void *d)
{
    uint32_t  n;
 ...
 */
static void timMergeForceCollapse(timMergeState *aState, cmpFunc *aCmpCb, void *d)
{
    int32_t  n;
    timSlice *sSlice = aState->mPendingRun;
    ...
    while (aState->mPendingRunCnt > 1)
    {
        ...
        timMergeAt(aState, (uint32_t)n, aCmpCb, d);
    }
}

void ruby_timsort(void *aArray, size_t aElementCnt, size_t aWidth, int (*aCmpCb)(const void *, const void *, void*), void *d)
{
    ...
    do
    {
        sRunLen = timCountRunAndMakeAscending(&sState, sIndexLow, sIndexHigh, sCmpCb, d);
        ...
        timMergeCollapse(&sState, sCmpCb, d);
     ...
     */
    timMergeForceCollapse(&sState, sCmpCb, d);
